...
// Then counts outliers from files with numeric names in ranges
// [a, a+RANGE_SIZE-1], [a+RANGE_SIZE, a+2*RANGE_SIZE-1], etc.
// Outlier detection uses data from row 22 to the last row of each file.
...
// --- CONFIGURATION ---
const int RANGE_START = 0;   // Starting file index 'a'
...
AnalysisResult process_range(int range_start, int current_sigma_threshold,
    ...
    for (int i = range_start; i < range_start + RANGE_SIZE; i++) {
        ...
        if (file_it != numeric_files.end()) {
            ...
            int outliers_in_file = count_outliers_beyond_nsigma(amplitude_data_full, global_mean, global_stddev, current_sigma_threshold);
            result.total_outliers += outliers_in_file;
            result.files_analyzed++;
            if (outliers_in_file > 0) {
                result.files_with_outliers++;
                std::cout << "File " << i << ".csv (" << filename << "): " << outliers_in_file << " outliers (using data from row 22 onwards)" << std::endl;
            } else {
                std::cout << "File " << i << ".csv (" << filename << "): No outliers (using data from row 22 onwards)" << std::endl;
            }
        }
    }
    ...
    std::cout << "  Total outliers beyond " << current_sigma_threshold << "-sigma (based on rows 22-98 stats): " << result.total_outliers << std::endl;
