...
// using ALL data points from row 22 onwards for files within that category only.
...
void all_points_kv_stats() {
    ...
    std::cout << "Statistics calculated using ALL rows from 22 onwards for each kV category separately." << std::endl;
    ...
    // Map to store amplitude data for each kV category (from ALL rows 22 onwards)
    std::map<int, std::vector<double>> kV_amplitude_data; // kV -> data points (ALL rows 22+)
    ...
    std::cout << "\n=== SECOND PASS: Aggregating data by kV category (using ALL rows 22 onwards) ===" << std::endl;
        ...
        if (kV_cat >= 8 && kV_cat <= 14) {
            ...
            if (amp_it != file_amplitudes.end()) {
                ...
                if (!file_data_full.empty()) {
                    // Add ALL the data from row 22 onwards to the kV category's data
                    kV_amplitude_data[kV_cat].insert(
                        kV_amplitude_data[kV_cat].end(),
                        file_data_full.begin(),
                        file_data_full.end()
                    );
                    kV_file_count[kV_cat]++;
                    std::cout << "Added data from file " << file_index << " (kV " << kV_cat << "). "
                              << "Used " << file_data_full.size() << " points (ALL rows 22 onwards)." << std::endl;
                } else {
    ...
    std::cout << "\n=== CALCULATING PER-kV STATISTICS (using ALL rows 22 onwards) ===" << std::endl;
    ...
    for (int kV = 8; kV <= 14; ++kV) {
        ...
        if (data_it != kV_amplitude_data.end() && !data_it->second.empty()) {
            ...
            std::cout << "  Data points (ALL rows 22 onwards): " << stats.data_points << std::endl;
    ...
    // --- Plot 1: Sigma vs kV ---
    TCanvas *c1 = new TCanvas("c1", "Sigma vs kV (All Points)", 800, 600);
    c1->SetGrid();

    TGraphErrors *graph_sigma = new TGraphErrors(n_points, x_vals.data(), sigma_vals.data(), x_errs.data(), nullptr); // No y-errors
    graph_sigma->SetTitle("Standard Deviation (Sigma) vs kV (All Points); kV; Sigma (Standard Deviation)");
    ...
    c1->Update();
    std::cout << "Plot 'c1' created: Sigma vs kV (All Points). You can interact with it or save it via the ROOT canvas menu." << std::endl;


     // --- Plot 2: Mean vs kV ---
    TCanvas *c2 = new TCanvas("c2", "Mean vs kV (All Points)", 800, 600);
    c2->SetGrid();

    TGraphErrors *graph_mean = new TGraphErrors(n_points, x_vals.data(), mean_vals.data(), x_errs.data(), nullptr); // No y-errors
    graph_mean->SetTitle("Mean vs kV (All Points); kV; Mean");
    ...
    c2->Update();
    std::cout << "Plot 'c2' created: Mean vs kV (All Points). You can interact with it or save it via the ROOT canvas menu." << std::endl;
