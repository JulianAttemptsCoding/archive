...
//   - Graph 1: Time (A) vs Trigger (Binned Presence) and Time (A) vs Ch1 (Binned Presence)
//   - Graph 2: Time (A) vs Trigger (Binned Presence) and Time (A) vs Ch2 (Binned Presence)
//   - Graph 3: Time (A) vs Trigger (Binned Presence) and Time (A) vs Ch3 (Binned Presence)
...
// Color now represents if a bin contains points from BOTH datasets (Trigger AND ChX) for that plot.
// All plots are saved to a SINGLE PAGE PDF file in a grid layout.
// The overlay shows this binary overlap for Trigger and ChX in the same plot.
...
void time_vs_columns_density_by_kv() {
    std::cout << "Starting analysis: Time vs Specific Columns with Binned OVERLAP Coloring (Single Page PDF), grouped by kV..." << std::endl;
    ...
    std::cout << "Plotting combinations using binned overlap (TH2F) for each kV (8-14):" << std::endl;
    std::cout << "  For each kV: 1. Time vs Trigger & Ch1, 2. Time vs Trigger & Ch2, 3. Time vs Trigger & Ch3" << std::endl;
    std::cout << "  Using " << NUM_BINS_X << " x " << NUM_BINS_Y << " bins for overlap representation." << std::endl;
    std::cout << "Color now represents if a bin contains points from BOTH datasets (1) or not (0)." << std::endl;
    std::cout << "Plots will be saved to 'time_vs_columns_density_by_kv_output.pdf' (ALL on ONE page)." << std::endl;
    std::cout << "Overlaid overlap plots. No global palette or large external labels." << std::endl;
    ...
    std::cout << "\n=== PASS 3: Creating overlap plots and saving to SINGLE PAGE PDF ===" << std::endl;
    // --- Configuration for Single Page Layout ---
    int n_kVs = sorted_kVs.size();
    int n_cols = 3; // Ch1, Ch2, Ch3 comparisons (Overlap shown)
    int n_rows = n_kVs;
    ...
    TCanvas *pdf_canvas = new TCanvas("pdf_canvas", "Combined Overlap Plots", canvas_width, canvas_height);
    ...
    for (int row = 0; row < n_rows; ++row) {
        ...
        for (int col = 0; col < n_cols; ++col) {
            ...
            // Use the same binning and ranges for all
            TString hist_name_base = Form("hBase_kv%d_col%d", kV, col);
            TH2F *hBase = new TH2F(hist_name_base, "", NUM_BINS_X, x_min, x_max, NUM_BINS_Y, y_min_overall, y_max_overall);

            // Histograms to store binary masks (1 if point exists in bin, 0 otherwise)
            TH2F *hTriggerMask = (TH2F*)hBase->Clone(Form("hTriggerMask_kv%d_col%d", kV, col));
            TH2F *hOtherMask = (TH2F*)hBase->Clone(Form("hOtherMask_kv%d_col%d", kV, col));

            // Histogram to store the final overlap result
            TH2F *hOverlap = (TH2F*)hBase->Clone(Form("hOverlap_kv%d_col%d", kV, col));

            TString other_label;
            std::vector<double> *other_data_ptr = nullptr;

            // Determine which channel data to use
            if (col == 0) {
                other_label = "Ch1";
            ...
            // --- Fill the Binary Mask Histograms and Calculate Overlap ---
            if (hTriggerMask && hOtherMask && hOverlap && other_data_ptr) {
                // Fill Trigger Mask
                for (size_t i = 0; i < kV_times.size(); ++i) {
                    if (is_valid_number(kV_times[i]) && is_valid_number(kV_Trigger[i])) {
                        hTriggerMask->Fill(kV_times[i], kV_Trigger[i]);
                    }
                }
                // Fill Other Mask
                for (size_t i = 0; i < kV_times.size(); ++i) {
                    if (is_valid_number(kV_times[i]) && is_valid_number((*other_data_ptr)[i])) {
                        hOtherMask->Fill(kV_times[i], (*other_data_ptr)[i]);
                    }
                }

                // --- Convert Counts to Binary Masks (0 or 1) and Calculate Overlap ---
                // Iterate over all bins
                for (int binx = 1; binx <= NUM_BINS_X; ++binx) {
                    for (int biny = 1; biny <= NUM_BINS_Y; ++biny) {
                         // Get bin content (count)
                        double trigger_count = hTriggerMask->GetBinContent(binx, biny);
                        double other_count = hOtherMask->GetBinContent(binx, biny);

                        // Set to 1 if count > 0, otherwise 0
                        bool trigger_has_point = (trigger_count > 0);
                        bool other_has_point = (other_count > 0);
                        hTriggerMask->SetBinContent(binx, biny, trigger_has_point ? 1.0 : 0.0);
                        hOtherMask->SetBinContent(binx, biny, other_has_point ? 1.0 : 0.0);

                        // --- Calculate Overlap ---
                        // 1 if both have points, 0 otherwise
                        double overlap_value = (trigger_has_point && other_has_point) ? 1.0 : 0.0;
                        hOverlap->SetBinContent(binx, biny, overlap_value);
                    }
                }


                // --- Style and Draw the Overlap Histogram ---
                // Title only shows kV for minimalism
                TString hist_title = Form("kV %d", kV);
                hOverlap->SetTitle(hist_title); // Set title on the final plot histogram
                hOverlap->SetStats(0); // Disable stats box

                // Minimal axis labels
                hOverlap->GetXaxis()->SetTitle("Time");
                hOverlap->GetYaxis()->SetTitle(Form("Amplitude (%s/Trig)", other_label.Data()));
                hOverlap->GetXaxis()->SetLabelSize(0.05);
                hOverlap->GetYaxis()->SetLabelSize(0.05);
                hOverlap->GetXaxis()->SetTitleSize(0.06);
                hOverlap->GetYaxis()->SetTitleSize(0.06);
                hOverlap->GetXaxis()->SetTitleOffset(0.9);
                hOverlap->GetYaxis()->SetTitleOffset(1.0);

                // Ensure the palette range is set correctly for 0 and 1
                hOverlap->SetMinimum(0.0);
                hOverlap->SetMaximum(1.0);

                // Draw the overlap result
                // Use COL to draw bins with color based on content (0 or 1)
                hOverlap->Draw("COL");

                std::cout << "  -> Created overlap plot for kV " << kV << "kV, Trigger vs " << other_label << " (Pad row=" << (row+1) << ", col=" << (col+1) << ")." << std::endl;
            } else {
                std::cerr << "  -> Error: Failed to create histograms for kV " << kV << "kV, column " << col << "." << std::endl;
            }

            // Clean up temporary histograms for this pad
            delete hBase; // hBase was the base, clones manage their own memory
            delete hTriggerMask;
            delete hOtherMask;
            // hOverlap is kept for drawing, will be cleaned with the pad/canvas eventually
    ...
    pdf_canvas->Update();
    TString pdf_filename = "time_vs_columns_density_by_kv_output.pdf"; // Filename unchanged
    pdf_canvas->Print(pdf_filename + "["); // Open PDF
    pdf_canvas->Print(pdf_filename);       // Add this page
    pdf_canvas->Print(pdf_filename + "]"); // Close PDF
    std::cout << "\nCombined PDF '" << pdf_filename << "' created successfully with ALL OVERLAP plots on ONE page." << std::endl;
    ...
    std::cout << "Color now represents overlap (1 if both datasets have points in bin, 0 otherwise)." << std::endl;
    // Clean up the main canvas
    delete pdf_canvas;
    std::cout << "\nOverlap analysis and single-page plotting complete for all kV categories." << std::endl;
    std::cout << "Note: Color now signifies if a bin contains points from BOTH datasets." << std::endl;
    std::cout << "      Resolution is still controlled by NUM_BINS_X and NUM_BINS_Y." << std::endl;
